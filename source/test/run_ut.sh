...
log "INFO" "Running make for CcspEthAgent_gtest.bin..."
...
if [ -f "./source/test/CcspEthAgent_gtest.bin" ]; then
    log "INFO" "Running CcspEthAgent_gtest.bin"
    ./source/test/CcspEthAgent_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "CcspEthAgent_gtest.bin does not exist, cannot run tests"
    exit 1
fi
