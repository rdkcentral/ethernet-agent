...
static void EthAgentLogDmRegMem(const char *stage)
{
    ...
    if (fp)
    {
        ...
        fprintf(fp, "%s pid=%d component=%s stage=%s%s\n",
                ts, (int)getpid(), CCSP_COMPONENT_NAME_ETHAGENT, stage, summary);
        fclose(fp);
    }
}
    ...
    )
{
    ...
    EthAgentLogDmRegMem("before_CpeController_Engage");
    ...
    EthAgentLogDmRegMem("after_CpeController_Engage");
    ...
    EthAgentLogDmRegMem("before_RegisterCcspDataModel2");
    ...
    EthAgentLogDmRegMem("after_RegisterCcspDataModel2");
